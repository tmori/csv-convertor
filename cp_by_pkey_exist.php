...
if (is_null($dump_path)) {
    $child_csv_obj->dump();
    $dump_csv_obj = new CsvFileIo('./dump.csv');
}
else {
    $child_csv_obj->dump($dump_path);
    $dump_csv_obj = new CsvFileIo($dump_path);
}
...
$dump_csv_obj->splice_all($start_line_child);
...
for ($i = $start_line_child; $i < $child_csv_obj->linenum(); $i++) {
    ...
    if ($keyword) {
        ...
        if ($row_id) {
            ...
            $child_line = $child_csv_obj->line($i);
            $dump_csv_obj->insert($child_line);
            $dump_csv_obj->set_value($i, $child_dst_col, $src_value);
        }
        else {
